...
public class AuthController {
	...
	@PostMapping("/signup")
	public String signup(@ModelAttribute @Validated SignupForm signupForm,BindingResult result,RedirectAttributes redirectAttributes,Model model,HttpServletRequest httpServletRequest) {
		
		// メールアドレスが登録済みであれば、BindingResultオブジェクトにエラー内容を追加する
		if(userService.isEmailRegistered(signupForm.getEmail())) {
			FieldError fieldError = new FieldError(result.getObjectName(),"email","既に登録済みのメールアドレスです。");
	...
	public String veify(@RequestParam(name = "token") String token,Model model) {
